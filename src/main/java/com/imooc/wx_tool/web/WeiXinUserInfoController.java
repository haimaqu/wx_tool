...
import com.imooc.wx_tool.entity.WeiXinUser;
import com.imooc.wx_tool.service.WeiXinUserInfoService;
import com.imooc.wx_tool.util.JsapiTicketUtil;
import com.imooc.wx_tool.util.WeiXinUtil;
...
public class WeiXinUserInfoController {
     ...
     * @param session
     * @param map
     * @return   <div id="top"><a class="go_back" href="./"><img th:src="@{${product.imgAddr}}"/></a>
     */

    @RequestMapping(value = "/tologinuserinfo")
    ...
    @RequestMapping(value = "/tologinuserinfos")
    public ModelAndView checks(HttpServletRequest request , HttpSession session, Map<String, Object> map) {
        // 首先判断一下session中，是否有保存着的当前用户的信息，有的话，就不需要进行重复请求信息
        WeiXinUser weiXinUser = null ;
        if(session.getAttribute("currentUser") != null){
            weiXinUser = (WeiXinUser) session.getAttribute("currentUser");
